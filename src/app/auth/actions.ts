...
import { z } from "zod";
import {
  isAuthorizedDashboardAuthUser,
  provisionLocalUserFromAuthUser,
} from "@/lib/auth";
import {
...
const inviteCodeSchema = z.string().trim().max(128);
...
export async function signInWithPasswordAction(formData: FormData) {
  ...
  if (!data.user || !(await isAuthorizedDashboardAuthUser(data.user))) {
...
export async function signUpWithPasswordAction(formData: FormData) {
  ...
  if (!canCreateAccountWithInvite(email, inviteCodeResult.data)) {
  ...
  try {
    if (data.user) {
      await provisionLocalUserFromAuthUser(data.user);
    }
  } catch {
    await supabase.auth.signOut();
    redirectToAuthError("/sign-up", "signup-failed", next);
